...
const ParseDuration = ({ isoDuration }) => {
  ...
  const formattedDuration = `${parsedHours > 0 ? parsedHours.toString().padStart(2, '0') + ':' : ''}${parsedMinutes.toString().padStart(2, '0')}:${parsedSeconds.toString().padStart(2, '0')}`;
  return (
...
export const DateDifference = ({inputDate}) => {
    ...
    useEffect(() => {
      ...
      if (yearsDifference > 0) {
        setDifference(`${yearsDifference} year${yearsDifference === 1 ? '' : 's'}`);
      } else if (monthsDifference > 0) {
        setDifference(`${monthsDifference} month${monthsDifference === 1 ? '' : 's'}`);
      } else if (daysDifference > 0) {
        setDifference(`${daysDifference} day${daysDifference === 1 ? '' : 's'}`);
      } else if (hoursDifference > 0) {
        setDifference(`${hoursDifference} hour${hoursDifference === 1 ? '' : 's'}`);
      } else if (minutesDifference > 0) {
        setDifference(`${minutesDifference} minute${minutesDifference === 1 ? '' : 's'}`);
      } else {
        setDifference(`${secondsDifference} second${secondsDifference === 1 ? '' : 's'}`);
      }
    }, []);
